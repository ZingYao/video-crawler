...
export class VirtualMouse {
    ...
    // 缩放适配
    private appScale = 1
    private scaleMode: 'none' | 'zoom' | 'transform' = 'none'
    // Enter 点击后短暂忽略真实鼠标事件，避免位置被物理鼠标“拉回”
    ...
    private setCursorPos(p: Vec2) {
        ...
        // 触发虚拟光标的鼠标事件
        this.triggerVirtualMouseEvents({ x: p.x, y: p.y })
    }

    // 触发虚拟光标的鼠标事件
    private triggerVirtualMouseEvents(pos: Vec2) {
        // 将视觉坐标换算为实际布局坐标，用于命中与事件 client 坐标
        const lp = this.toRealPoint(pos.x, pos.y)
        const element = document.elementFromPoint(lp.x, lp.y)
        if (!element) return
    ...
    private loop = () => {
        ...
        const size = this.opts.cursorSize
        // 边界：使用视觉坐标空间
        const { w: vW, h: vH } = this.getVisualWindowSize()
        const maxX = Math.max(1, Math.floor(vW)) - 1
        const maxY = Math.max(1, Math.floor(vH)) - 1
        // 更新位置 + 边界限制（中心点不越界）
    ...
    private scrollByDelta(delta: number) {
        const lp = this.toRealPoint(this.pos.x, this.pos.y)
        const elAtPointer = document.elementFromPoint(lp.x, lp.y)
    ...
    private onKeyDown = (e: KeyboardEvent) => {
        ...
        // Enter：在当前虚拟光标位置触发点击（包含 input/slider 等）
        if (normKey === 'Enter') {
            const lp = this.toRealPoint(this.pos.x, this.pos.y)
            // 这里需要计算实际坐标，然后点击实际坐标
            vmLog('[VM][keydown] Enter 键触发点击:', {
                pos: { ...lp },
                element: document.elementFromPoint(lp.x, lp.y) as any,
                timestamp: Date.now()
            })
            // Enter 后 600ms 内忽略真实鼠标事件，保持键盘模式位置不被物理鼠标影响
            this.isKeyboardMode = true
            this.hiddenByMouse = false
            this.ignoreMouseUntil = Date.now() + 600
            e.preventDefault(); e.stopPropagation()
            const el = this.elementFromVisualPoint(lp.x, lp.y) as HTMLElement | null
            if (el) {
                const pageX = lp.x + window.scrollX
                const pageY = lp.y + window.scrollY
                const screenX = (window.screenX || 0) + lp.x
                const screenY = (window.screenY || 0) + lp.y
                try {
                    const rect = el.getBoundingClientRect()
                    console.log('[VM][CLICK] dispatch', {
                        visual: JSON.stringify({ x: Math.round(this.pos.x), y: Math.round(this.pos.y) }),
                        layout: JSON.stringify({ x: Math.round(lp.x), y: Math.round(lp.y) }),
                        page: JSON.stringify({ x: Math.round(pageX), y: Math.round(pageY) }),
                        screen: JSON.stringify({ x: Math.round(screenX), y: Math.round(screenY) }),
                        mouseRaw: JSON.stringify({ x: Math.round(this.lastMousePos.x), y: Math.round(this.lastMousePos.y) }),
                        target: el,
                        targetRect: JSON.stringify({ x: Math.round(rect.left), y: Math.round(rect.top), w: Math.round(rect.width), h: Math.round(rect.height) }),
                        scaleMode: this.scaleMode,
                        scale: this.appScale,
                    })
                } catch { }
                const ev: MouseEventInit = { bubbles: true, cancelable: true, composed: true, clientX: lp.x, clientY: lp.y, screenX, screenY, button: 0, buttons: 1 as any }
                try { el.dispatchEvent(new PointerEvent('pointerdown', { ...ev, pointerType: 'mouse', isPrimary: true, buttons: 1 })) } catch { }
                const downOk = el.dispatchEvent(new MouseEvent('mousedown', ev))
                try { el.dispatchEvent(new PointerEvent('pointerup', { ...ev, pointerType: 'mouse', isPrimary: true, buttons: 0 })) } catch { }
                const upOk = el.dispatchEvent(new MouseEvent('mouseup', ev))
                const clickOk = el.dispatchEvent(new MouseEvent('click', ev))
                if ((el as any).click && (!downOk || !upOk || !clickOk)) {
                ...
                // 2) 原生 range：按点击位置计算并设置新值，然后触发 input/change
                if (tag === 'INPUT' && type === 'range') {
                    const input = el as HTMLInputElement
                    const min = isNaN(Number(input.min)) ? 0 : Number(input.min)
                    const max = isNaN(Number(input.max)) ? 100 : Number(input.max)
                    const step = Math.abs(Number(input.step)) || 1
                    const rect = input.getBoundingClientRect()
                    const ratio = rect.width > 0 ? Math.min(1, Math.max(0, (lp.x - rect.left) / rect.width)) : 0
                ...
                // 3) 无障碍 slider（如 Ant Design）：尝试对 handle 派发指针事件，并确保获取焦点
                if (role === 'slider') {
                    try { (el as any).focus?.({ preventScroll: true }) } catch {}
                    const rect = (el as HTMLElement).getBoundingClientRect()
                    const handle = (el as HTMLElement).querySelector?.('.ant-slider-handle') as HTMLElement | null
                    if (handle) {
                        const hev: MouseEventInit = { bubbles: true, cancelable: true, composed: true, clientX: lp.x, clientY: Math.min(Math.max(lp.y, rect.top + 1), rect.bottom - 1), screenX, screenY, button: 0, buttons: 1 as any }
                        try { handle.dispatchEvent(new PointerEvent('pointerdown', { ...hev, pointerType: 'mouse', isPrimary: true, buttons: 1 })) } catch {}
                        handle.dispatchEvent(new MouseEvent('mousedown', hev))
                        try { handle.dispatchEvent(new PointerEvent('pointerup', { ...hev, pointerType: 'mouse', isPrimary: true, buttons: 0 })) } catch {}
                        handle.dispatchEvent(new MouseEvent('mouseup', hev))
                        handle.dispatchEvent(new MouseEvent('click', hev))
                    } else {
                        // 没有 handle 时，至少补发一次 input/change 让受控组件响应
    ...
    private onMouseActivity = (e: Event) => {
        // 跳过所有非可信(合成)鼠标事件，防止 Enter 派发的 click/mousedown 等影响位置
        if (e instanceof MouseEvent && e.isTrusted === false) {
            return
        }
        // Enter 后的一小段时间忽略真实鼠标事件，防止回到物理鼠标位置
        if (e instanceof MouseEvent && this.ignoreMouseUntil > Date.now()) {
            return
        }
        ...
        if (e instanceof MouseEvent) {
            // 若鼠标移出可视区域，立即隐藏虚拟光标
            const { w: limitW, h: limitH } = this.getVisualWindowSize()
            const outOfViewport = (
                e.clientX < 0 || e.clientY < 0 ||
                e.clientX > limitW || e.clientY > limitH
            )
            if (outOfViewport) {
            ...
            // 将鼠标事件坐标转换为视觉坐标
            const vpt = this.toVisualPoint(e.clientX, e.clientY)
            //   try {
            //     console.log('[VM][MOUSE] move', {
            //       raw: JSON.stringify({ x: Math.round(e.clientX), y: Math.round(e.clientY) }),
            //       visual: JSON.stringify({ x: Math.round(vpt.x), y: Math.round(vpt.y) }),
            //       vm: JSON.stringify({ x: Math.round(this.pos.x), y: Math.round(this.pos.y) }),
            //       scaleMode: this.scaleMode,
            //       scale: this.appScale
            //     })
            //   } catch {}
            this.lastMousePos.x = vpt.x
            this.lastMousePos.y = vpt.y
            this.pos.x = vpt.x
            this.pos.y = vpt.y
            // 鼠标左键按下时输出详细信息
            if (e.type === 'mousedown' && e.button === 0) {
                try {
                    ...
                    console.log('[VM][MOUSE_DOWN]', {
                        raw: JSON.stringify({ x: Math.round(e.clientX), y: Math.round(e.clientY) }),
                        visual: JSON.stringify({ x: Math.round(vpt.x), y: Math.round(vpt.y) }),
                        vm: JSON.stringify({ x: Math.round(this.pos.x), y: Math.round(this.pos.y) }),
                        target,
                        targetRect: rect ? JSON.stringify({ x: Math.round(rect.left), y: Math.round(rect.top), w: Math.round(rect.width), h: Math.round(rect.height) }) : 'null',
                        scaleMode: this.scaleMode,
                        scale: this.appScale
                    })
                } catch { }
    ...
    // ===== 缩放支持工具 =====
    private updateScaleFromDOM() {
        const root = document.documentElement as HTMLElement
        // 读取 transform: matrix(...) 的 scale
        const cs = getComputedStyle(root)
        const tr = (root.style.transform || cs.transform || '').trim()
        let scaleFromTransform = 1
        if (tr && tr !== 'none') {
            ...
            if (m && m[1]) {
                ...
                if (parts.length >= 6) {
                    ...
                    scaleFromTransform = Math.sqrt((a || 1) * (a || 1) + (b || 0) * (b || 0)) || 1
                }
            }
        }
        // 读取 zoom（部分浏览器支持）
        const zmInline: any = (root.style as any).zoom
        const zmComputed: any = (cs as any).zoom
        const scaleFromZoom = Number(zmInline || zmComputed || 1) || 1
        // 综合判断
        if (scaleFromTransform && Math.abs(scaleFromTransform - 1) > 0.001) {
            this.scaleMode = 'transform'
            this.appScale = scaleFromTransform
        } else if (scaleFromZoom && Math.abs(scaleFromZoom - 1) > 0.001) {
            this.scaleMode = 'zoom'
            this.appScale = scaleFromZoom
        } else {
            this.scaleMode = 'none'
            this.appScale = 1
        }
    ...
    private onAppScaleChanged = (_e: Event) => {
        const prev = this.appScale
        ...
        const maxX = Math.max(1, Math.floor(vW)) - 1
        const maxY = Math.max(1, Math.floor(vH)) - 1
        this.pos.x = clamp(this.pos.x, size / 2, maxX - size / 2)
        this.pos.y = clamp(this.pos.y, size / 2, maxY - size / 2)
        this.setCursorPos(this.pos)
        vmLog('[VM][scale] app-scale-changed', { prev, next: this.appScale, prevMode, nextMode: this.scaleMode })
    }

    // 统一：视觉坐标 -> 布局坐标
    private toLayoutPoint(x: number, y: number): { x: number; y: number } {
        const s = this.appScale || 1
        if (!s || Math.abs(s - 1) < 1e-3) return { x, y }
        // 对 transform 与 zoom 都进行除法换算
        return { x: x / s, y: y / s }
    }

    private toRealPoint(x: number, y: number): { x: number; y: number } {
        const s = this.appScale || 1
        if (!s || Math.abs(s - 1) < 1e-3) return { x, y }
        return { x: x * s, y: y * s }
    }
    ...
    // 统一：根据缩放，从视觉坐标获取命中元素
    private elementFromVisualPoint(x: number, y: number): Element | null {
        return document.elementFromPoint(x, y)
    }

    // 统一：布局坐标 -> 视觉坐标（用于将鼠标事件位置映射到虚拟光标的视觉空间）
    private toVisualPoint(x: number, y: number): { x: number; y: number } {
        const s = this.appScale || 1
        if (!s || Math.abs(s - 1) < 1e-3) return { x, y }
        // 对于 zoom/transform，事件坐标通常以布局坐标呈现，将其折算为视觉坐标
        return { x: x / s, y: y / s }
    }

    // 统一：获取视觉窗口尺寸
    private getVisualWindowSize(): { w: number; h: number } {
        const s = this.appScale || 1
        if (!s || Math.abs(s - 1) < 1e-3) return { w: window.innerWidth, h: window.innerHeight }
        // 视觉坐标空间：等于布局宽高 / scale（缩小视觉像素密度）
...
export const enableVirtualMouse = (opts?: VirtualMouseOptions) => {
    ...
    try { window.addEventListener('app-scale-changed', (globalVirtualMouse as any).onAppScaleChanged as any) } catch { }
...
export const disableVirtualMouse = () => {
    if (globalVirtualMouse) {
        try { window.removeEventListener('app-scale-changed', (globalVirtualMouse as any).onAppScaleChanged as any) } catch { }
        globalVirtualMouse.disable()
