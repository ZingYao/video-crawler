...
func (e *LuaEngine) registerFunctions() {
	...
	e.L.SetGlobal("package", e.createDisabledTable("package"))
...
// createDisabledTable 直接创建包含禁用方法的表
func (e *LuaEngine) createDisabledTable(tableName string) *lua.LTable {
	// 创建一个表，包含所有被禁用的方法
	table := e.L.CreateTable(0, 10)

	// 为每个可能的方法添加禁用函数
	disabledMethods := []string{"open", "popen", "close", "read", "write", "flush", "seek", "lines", "input", "output"}
	for _, method := range disabledMethods {
		table.RawSetString(method, e.L.NewFunction(func(L *lua.LState) int {
